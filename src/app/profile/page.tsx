...
export default function ProfilePage() {
 ...
 <UserCircle className="mr-3 h-8 w-8" /> โปรไฟล์ของฉัน
        </h1>
 <p className="text-muted-foreground mt-1">ดูและจัดการรายละเอียดบัญชีของคุณ</p>
      </header>

      <Card className="max-w-3xl mx-auto">
        <CardHeader className="flex flex-col sm:flex-row items-center gap-6 p-6">
          <Avatar className="h-24 w-24 border-2 border-primary">
 <AvatarImage src={user.profilePictureUrl} alt={user.name} data-ai-hint="รูปภาพบุคคล" />
 ...
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
 <Label htmlFor="profileName">ชื่อเต็ม</Label>
                <Input id="profileName" defaultValue={user.name} readOnly />
              </div>
              <div>
 <Label htmlFor="profileEmail">ที่อยู่อีเมล</Label>
                <Input id="profileEmail" type="email" defaultValue={user.email} readOnly />
              </div>
              <div>
 <Label htmlFor="profilePhone">เบอร์โทรศัพท์</Label>
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-semibold font-headline">Address Information</h3>
             <div>
 <Label htmlFor="profileAddress">ที่อยู่หลัก</Label>
